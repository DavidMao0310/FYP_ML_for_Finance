...
class AccelIncrease(BaseEstimator, TransformerMixin):
    ...
    def transform(self, df, y=None):
        ...
        for col in self.cols:
            ...
            df[str(col) + '_accstreak'] = df['increase'].groupby(
                ((df['increase'] != df['increase'].shift())).cumsum()).cumsum() \
...
class StreakIncrease(BaseEstimator, TransformerMixin):
    ...
    def transform(self, df, y=None):
        ...
        for col in self.cols:
            df['gt'] = df[col] > self.x
            df[str(col) + '_streakgt_' + str(self.x)] = df['gt'].groupby(
                ((df['gt'] != df['gt'].shift())).cumsum()).cumsum() \
                .where(df['symbol'] == df['symbol'].shift())
